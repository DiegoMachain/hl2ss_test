...
#Cut the dataset with the pre-defined bounding box
...
#print("Parent directory = ", os.path.dirname(directory))
#Change the directory so it finds the correct folders for src
...
#We need to load the model only once!
...
#View the results of Ditto model
import trimesh
...
#Here we need to send back the result to the hololens subscriber
...
#Try to fix the mesh if the object is composed of only flat surfaces
import pymeshfix
scene = trimesh.Scene()
scene_2 = trimesh.Scene()
static_mesh = mesh_dict[0].copy()
mobile_mesh = mesh_dict[1].copy()

#Create static mesh

static_pcd = o3d.geometry.PointCloud()
static_pcd.points = o3d.utility.Vector3dVector(np.asarray(static_mesh.vertices))
static_pcd = sum_downsample_points([static_pcd], 0.01, 50, 0.1)

#Create mobile mesh to fix 
mobile_pcd = o3d.geometry.PointCloud()
mobile_pcd.points = o3d.utility.Vector3dVector(np.asarray(mobile_mesh.vertices))
mobile_pcd = sum_downsample_points([mobile_pcd], 0.01, 50, 0.1)

#Create point cloud
alpha = .5
print(f"alpha={alpha:.3f}")
static_pcd = o3d.geometry.TriangleMesh.create_from_point_cloud_alpha_shape(static_pcd, alpha)
static_pcd.compute_vertex_normals()


alpha = .5
print(f"alpha={alpha:.3f}")
mobile_pcd = o3d.geometry.TriangleMesh.create_from_point_cloud_alpha_shape(mobile_pcd, alpha)
mobile_pcd.compute_vertex_normals()

#Improvement of meshes
fixed_static_mesh = pymeshfix.MeshFix(np.asarray(static_pcd.vertices), np.asarray(static_pcd.triangles))
...
fixed_mobile_mesh = pymeshfix.MeshFix(np.asarray(mobile_pcd.vertices), np.asarray(mobile_pcd.triangles))
...
#Original mesh
scene.add_geometry(static_mesh)
scene.add_geometry(mobile_mesh)
